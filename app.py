# app.py (起動時にモデルを強制学習し、即時レポートを送信する完全版 - 日本語通知)
...
def run_prediction_and_notify():
    ...
    if bot is None:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🚨 BOTインスタンスがありません。タスクをスキップします。")
        return

    try:
        # モデルファイルが存在しない場合は、予測をスキップする（初回強制実行でカバーされているはず）
        if not os.path.exists(MODEL_FILENAME):
            print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ モデルファイル '{MODEL_FILENAME}' が存在しません。予測をスキップし、再学習を待ちます。")
            return

        print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚙️ 予測タスクを開始します...")
        
        # 高度な分析指標を取得
        # futures_ml_botからインポートされた関数を使用
...
def run_retrain_and_improve():
    """モデルの再学習と構築を行う関数"""
    if bot is None:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🚨 BOTインスタンスがありません。再学習をスキップします。")
        return
        
    try:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🧠 再学習タスクを開始します...")
...
def start_scheduler():
    ...
    # 🚨 1. 初回モデル構築を強制実行 (ファイルが存在しない場合に備えて必須)
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🚀 初回モデル構築を強制実行中です...")
    run_retrain_and_improve()
    print("✅ 初回モデル構築が完了しました。")

    # 🚨 2. モデル構築後、すぐに初回予測を実行しレポートを送信 (即時レポート機能)
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🚀 初回レポートを強制実行中です...")
